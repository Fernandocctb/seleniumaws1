...
# Fixture methdi - to close browser
def tearDown():
    if driver is not None:
        ...
        driver.close()
        driver.quit()
        # Make a log file with a dynamic fake value
...
# Log in method custom (dynamic) username and passwors
...
def create_new_user():
    ...
    # Click optional link to open that section
    ...
    sleep(0.25)
    # Fill out the ICQ Number open field
    ...
    sleep(0.25)
    # Fill out Mobile Phone input open field
...
def check_user_created():
    ...
    if driver.current_url == locators.moodle_users_main_page:
        assert driver.find_element(By.XPATH, "//h1[text() = 'Software Quality Assurance']").is_displayed()
        if driver.find_element(By.ID, 'fgroup_id_email_grp_label') and \
            driver.find_element(By.NAME, 'email'):
            sleep(0.25)
            driver.find_element(By.CSS_SELECTOR, 'input#id_email').send.keys(locators.email)
            sleep(0.25)
...
def delete_a_new_user():
        ...
        if driver.find_element(By.ID, 'fgroup_id_email_grp_label') and \
            ...
            if driver.find_element(By.XPATH, f'//td[contains(., "{locators.email}")]'):
                ...
                print('---New User found and deleted ---Passed!')
#                Log out from Moodle app
#                log_out
...
# setUp()
# # Log in
# log_in(locators.practice_moodle_username, locators.practice_moodle_password)
# # Create a new user
# create_new_user()
...
# #tearDown()
# #sleeps(5)
# ### Log In with a New User Credentials Test Case ###
# # Open web browser
# #setUp()
# # log in
...
#     driver.find_element(By.LINK_TEXT, 'Browse list of users').click()
#     sleep(0.5)
#     if driver.find_element(By.LINK_TEXT, 'fgroup_id_email_grp_label').is_displayed() and \
#         driver.find_element(By.LINK_TEXT, 'id_email' ).is_displayed():
#         sleep(0.25)
#         driver.find_element(By.LINK_TEXT, 'id_addfilter').click()
#         sleep(0.25)
#         if driver.find_element(By.XPATH, 'f//td[conatins(., "{locators.email}")]').is_displayed() and \
#             driver.find_element(By.XPATH, '//*[@title="Delete"]').is_displayed():
#         driver.find_element(By.XPATH, '//*[@title="Delete"]').click()
#         sleep(0.25)
#         driver.find_element(By.XPATH, '//button[contains(., "Delete")]').click()
#         print(f'User with email {locators.email} got dleeted.')
#
#     else:
